...
# define FAT12          FAT12_nec
# define FAT16SMALL     FAT16SMALL_nec
# define FAT16LARGE     FAT16LARGE_nec
# define FAT32          FAT32_nec
# define FAT32_LBA      FAT32LBA_nec
#else
/* IBMPC FDISK (called as `msdos' on some patitioning tools) */
# define FAT12          FAT12_ibm
# define FAT16SMALL     FAT16SMALL_ibm
# define EXTENDED       EXTENDED_ibm
# define FAT16LARGE     FAT16LARGE_ibm
# define FAT32          FAT32_ibm
# define FAT32_LBA      FAT32_LBA_ibm
# define FAT16_LBA      FAT16LBA_ibm
#endif
...
#if defined(NEC98)
/* NEC PC-98x1 (pc98) */
# define IsLBAPartition(parttyp) IsLBAPartition_nec(parttyp)
# ifdef WITHFAT32
#  define IsFATPartition(parttyp)  IsFATPart32_nec(parttyp)
# else
#  define IsFATPartition(parttyp)  IsFATPart16_nec(parttyp)
# endif
#else
/* IBMPC FDISK (msdos) */
# define IsExtPartition(parttyp)    IsExtPartition_ibm(parttyp)
# define IsLBAPartition(parttyp)    IsLBAPartition_ibm(parttyp)
# ifdef WITHFAT32
#  define IsFATPartition(parttyp)    IsFATPart32_ibm(parttyp)
# else
#  define IsFATPartition(parttyp)    IsFATPart16_ibm(parttyp)
# endif
#endif
...
int Read1LBASector(struct DriveParamS *driveParam, unsigned drive, ULONG LBA_address, void * buffer);
                        ...
                        int extendedPartNo, int PrimaryNum)
{
  ...
  UBYTE globalLBA = InitKernelConfig.GlobalEnableLBAsupport;
  /* todo */
  ...
  /* get actual sector size (for block device, not always same for BIOS) from the disk */
    UWORD rc = globalLBA ? Read1LBASector(driveParam, driveParam->driveno, pEntry->RelSect, InitDiskTransferBuffer)
                         : fl_read(driveParam->driveno, pEntry->Begin.Head, pEntry->Begin.Cylinder, pEntry->Begin.Sector, 1024 /* 512 MAX_SEC_SIZE */, (UBYTE FAR *) InitDiskTransferBuffer);
...
PartitionsField ProcessDisk(int scanType, unsigned drive, PartitionsField PartitionsToIgnore)
{
  ...
  else
  {
    ...
    if (driveParam.sector_size == 512 && InitDiskTransferBuffer[0x1fe] == 0x55 && InitDiskTransferBuffer[0x1ff] == 0xaa)
