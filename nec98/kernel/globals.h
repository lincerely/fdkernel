...
#if defined(NEC98)
VOID ASMPASCAL ReadPC98Clock(VOID FAR *);
VOID ASMPASCAL WritePC98Clock(VOID FAR *);
#elif defined(IBMPC)
ULONG ASMPASCAL ReadPCClock(VOID);
VOID ASMPASCAL WriteATClock(BYTE *, BYTE, BYTE, BYTE);
VOID ASMPASCAL WritePCClock(ULONG);
#else
#error need specific Clock functions.
#endif
#ifdef __WATCOMC__
# if defined(NEC98)
#pragma aux (pascal) ReadPC98Clock modify exact [ax bx]
#pragma aux (pascal) WritePC98Clock modify exact [ax bx]
# elif defined(IBMPC)
...
# else
# error need specific Clock functions (pragmas for watcom).
# endif
#endif
